...
- `GET /health` -> état général du serveur et de la base locale
- `GET /api/health` -> santé API avec statut de la base et du fallback
...
Le serveur garde `server.js` comme point d'entree principal. Une partie de la logique est extraite dans des modules de routes et de services, mais la plupart des endpoints HTTP restent centralises dans le serveur principal.

- `server/routes/system.js` -> health checks et endpoints systeme
- `server/routes/advancedPrediction.js` -> prediction avancee
- `server/routes/aiPrediction.js` -> prediction IA via API
