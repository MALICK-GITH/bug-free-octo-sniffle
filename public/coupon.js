...
async function renderServerHistoryPanel() {
  ...
  try {
    ...
    title.textContent = "Historique serveur";
...
async function loadLeagues() {
  try {
    ...
    const sourceMatches = Array.isArray(payload?.matches) ? payload.matches : Array.isArray(data?.matches) ? data.matches : [];
    const leagues = [...new Set(sourceMatches.map((m) => String(m.league || "").trim()).filter(Boolean))].sort(
...
async function enforceTicketShield(actionLabel = "action") {
  const softExportImage = String(actionLabel || "").toLowerCase() === "export image";
  ...
  if (adapted.blocked.length > 0) {
    if (softExportImage) {
      lastCouponData = {
        ...lastCouponData,
        warning: `Export image autorise avec alerte Ticket Shield: ${adapted.blocked.join(" | ")}.`,
      };
      renderCoupon(lastCouponData);
      pushAlert({
        severity: "medium",
        title: "Ticket Shield",
        detail: `Export image autorise malgre blocage: ${adapted.blocked.join(" | ")}`,
        type: "ticket_shield_export_image_soft",
      });
      return adapted;
    }
